...
function Countries() {
  ...
  const ignoreCountries = ['Europe', 'North America', 'Asia', 'South America', 'Oceania', 'Africa'];

  useEffect(() => {
    fetch('https://coronavirus-19-api.herokuapp.com/countries')
        .then(res => res.json())
        .then(data => {
          setCountries(data.filter(({country})  => lib.ignore(country, ignoreCountries)).sort((a, b) => sortObj.default ? sortNumber(a[sortObj.value], b[sortObj.value]) : sortByCountry(a.country, b.country) ));
          setSave(data.filter(({country})  => lib.ignore(country, ignoreCountries)).sort((a, b) => sortObj.default ? sortNumber(a[sortObj.value], b[sortObj.value]) : sortByCountry(a.country, b.country) ));
          setIsLoader(false);
        })
  }, []);
  ...
  function sortCountries(e) {
    ...
    if(val === 'country') {
      const newData = [...save.sort((a, b) => sortByCountry(a[val], b[val]) )]
      setCountries(newData);
    }else {
      const newData = [...save.sort((a, b) => sortNumber(a[val], b[val]) )]
      setCountries(newData);
    }
  }
          ...
          isLoader ?
              ...
              <div className='countries' style={!isLoader ? {"overflowY": "scroll"}: null} >
               <div className='search'>
                 ...
                 <div className="sort-country-wrap">
                   <select className='sort-country'
                       onChange={sortCountries}
                       onFocus={
                         e => e.target.parentElement.classList.add('white-title')
                       }
                       onBlur={
                         e => e.target.parentElement.classList.remove('white-title')
                       }>
                     <option value='cases'>заболевшим</option>
                     <option value='country'>алфавиту</option>
                     <option value='todayCases'>заболевшим сегодня</option>
                     <option value='deaths'>смертям</option>
                     <option value='todayDeaths'>смертям сегодня</option>
...

               {
                 countries.map(({country, cases, todayCases, deaths, todayDeaths,
                                  recovered, active, critical, casesPerOneMillion},  i)=> {
